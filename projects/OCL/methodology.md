...
![Binary Constraints](assets/binary.png)
   ...
   - **Tensile Strength Constraint (Rm)**:
   
     The tensile strength (Rm) is predicted using the machine learning model previously introduced. We want to ensure that the predicted Rm meets or exceeds a minimum threshold Rm_min
​
The constraint can be formulated as:

![Tensile Strength Constraint](assets/Rm.png)

   - **Elongation Constraint (All)**:
   
     Similarly, elongation (All) is predicted and must meet or exceed a certain threshold All_min
